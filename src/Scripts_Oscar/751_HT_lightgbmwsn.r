...
PARAM <- list()
PARAM$experimento <- "HT7510wsn"

PARAM$exp_input <- "TS7410wsn"

# En caso que se haga cross validation, se usa esta cantidad de folds
PARAM$lgb_crossvalidation_folds <- 5

PARAM$lgb_semilla <- 829093 # cambiar por su propia semilla
...
PARAM$bo_lgb <- makeParamSet(
  ...
  makeIntegerParam("lambda_l1", lower = 0, upper = 200),
  makeIntegerParam("lambda_l2", lower = 0, upper = 200),
  makeIntegerParam("min_gain_to_split", lower = 0, upper = 1)
)
